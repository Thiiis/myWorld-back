...
public class AttachmentsService {
  ...
  public List<AttachmentsResponse> createAttach(Long did, List<MultipartFile> files) {
    ...
    for (MultipartFile file : files) {
      ...
      response.setUrl(publicBaseUrl + "/diaries/" + did + "/attache/file/" + attachment.getAid());  //AttachmentController에서 받는 URL요청
      // 2-4. 객체 List에 담기
  ...
  public AttachmentsResponse getAttache(Long did, Long aid) {
    ...
    response.setUrl(publicBaseUrl + "/diaries/" + did + "/attache/file/" + att.getAid());
  ...
  public List<AttachmentsResponse> getAttachmentsByDiary(Long did) {
    return attachmentDao.selectAttachByDid(did).stream().map(a -> new AttachmentsResponse(a.getAid(), a.getAname(), a.getAtype(), publicBaseUrl + "/diaries/" + did + "/attache/file/" + a.getAid())).collect(Collectors.toList());
  }
